...
def create_db_tag_list_for_comparison():
	...
	with con:
		cur = con.cursor()
		cur.execute("SELECT tag FROM players")
		rows = cur.fetchall()
		for row in rows:
...
def fuzzy_string_match_ratio(tourney_tag_list, db_tag_list):
	...
	for tourney_tag in tourney_tag_list:
		...
		for database_tag in db_tag_list:
			similar_ratio = fuzz.ratio(tourney_tag, database_tag)
			...
			if re.sub(' ', '', tourney_tag) == re.sub(' ', '', database_tag):
...
def check_for_alternate_tags(final_tag_list):
	...
	with con:
		cur = con.cursor()
		cur.execute("SELECT alt_tag, db_player_id FROM alternate_tags")
		rows = cur.fetchall()
		for row in rows:
		...
		for final_tag in final_tag_list:
			if not final_tag[1] and (final_tag[0] in alternate_tags_dict.keys()):
				...
				cur.execute("SELECT tag FROM players WHERE id=?", (player_id,))
				rows = cur.fetchone()
				print "rows", rows
				print "rows[0]", rows[0]
				final_tag = (rows[0], True)
		...
		for player_dict in challonge_player_dicts:
			...
			for original_challonge_tag, updated_database_tag in zip(list_of_player_tags, final_tag_list):
				if original_challonge_tag == player_dict["name"]:
					...
					cur.execute("SELECT * FROM players WHERE tag=?", (updated_database_tag[0],))
					rows = cur.fetchall()
					for row in rows:
						print row
					...
					if (len(rows) < 1) and not updated_database_tag[1]:
						...
						cur.execute("INSERT INTO players(tag, display_tag, trueskill_mu, trueskill_sigma, location) VALUES (?, ?, ?, ?, ?)", player_tuple)
