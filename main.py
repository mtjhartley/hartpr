...
def createSetsDictionary(player_id, ForIndex = True):
	...
	for row in rows:
		...
		if row[3] == player_id:
			...
			if winner_score and winner_score < 4 and loser_score < 3 and loser_score > -1:
...
def getSkillDistribution(rows, player_id = -1):
	...
	print "len(counts)/2"
	print len(counts)/2
	print "sorted filter lambda count: count > 0, counts"
	final_list = sorted(filter(lambda count: count > 0, counts))
	half = final_list[len(final_list)/2]
...
def getActivityRequirementDate():
	activityRequirementDate = str((datetime.datetime.now() - datetime.timedelta(days=240)).date())
